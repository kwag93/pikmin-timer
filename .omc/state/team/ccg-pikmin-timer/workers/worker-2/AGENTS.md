...
```bash
mkdir -p $(dirname .omc/state/team/ccg-pikmin-timer/workers/worker-2/.ready) && touch .omc/state/team/ccg-pikmin-timer/workers/worker-2/.ready
```
...
- **Worker**: worker-2
- **Agent Type**: gemini
- **Environment**: OMC_TEAM_WORKER=ccg-pikmin-timer/worker-2
...
- **Inbox**: Read .omc/state/team/ccg-pikmin-timer/workers/worker-2/inbox.md for new instructions
- **Heartbeat**: Update .omc/state/team/ccg-pikmin-timer/workers/worker-2/heartbeat.json every few minutes:
  ```json
...
- Path: .omc/state/team/ccg-pikmin-timer/workers/worker-2/done.json
...
2. Write an ACK file: .omc/state/team/ccg-pikmin-timer/workers/worker-2/shutdown-ack.json
3. Exit
